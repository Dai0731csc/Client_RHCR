(function initCameraMath(ns) {
  ...
  if (!mat3Api || !vec3Api || !quatApi) {
    throw new Error("gl-matrix is required for pose.js");
  }
...
(function initCameraIntrinsics(ns) {
  ...
  if (!z) {
    throw new Error("zod is required for pose.js");
  }
  ...
  function hasUsableCalibrationRecord(frameWidth, frameHeight, settings = {}) {
    return Boolean(
      getExternalIntrinsicsRecord(frameWidth, frameHeight, settings) ||
      getStoredIntrinsicsRecord(frameWidth, frameHeight, settings)
    );
  ...
  async function syncCalibrationFromBackend() {
    ...
    if (!response.ok || !result.success) {
      throw new Error(result.message || result.error || `Device profile request failed with ${response.status}`);
    }

    const device = result.device || null;
    state.deviceProfile = device;
    state.deviceHasCameraCalibration = Boolean(device?.camera_calibration);
    if (device?.camera_calibration) {
      saveCalibrationResult(device.camera_calibration);
    }
  ...
  ns.intrinsics = {
    ...
    hasUsableCalibrationRecord,
    parseCalibrationResult,
    saveCalibrationResult,
    syncCalibrationFromBackend,
  };
})(window.CameraPage = window.CameraPage || {});
