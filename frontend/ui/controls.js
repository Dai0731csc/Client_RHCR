(function initCameraUI(ns) {
  ...
  function setCameraUI(opened) {
    ...
    const hasCameraCalibration = Boolean(state.deviceHasCameraCalibration);
    ...
    dom.detectTagBtn.disabled = !opened || busy || !hasCameraCalibration;
    dom.timeSyncBtn.disabled = !opened || busy;
    dom.calibrateBtn.disabled = !opened || busy;
    setButtonVisual(
      dom.detectTagBtn,
      state.detectionLoopActive ? "stop" : "play",
      state.detectionLoopActive
        ? "Stop continuous detection"
        : hasCameraCalibration
          ? "Start continuous detection"
          : "Start continuous detection (run camera calibration first)"
    );
    setButtonVisual(
      dom.timeSyncBtn,
      "sync",
    ...
    setButtonVisual(
      dom.calibrateBtn,
      "target",
      state.calibrationInProgress ? "Initial calibration in progress..." : "Initial calibration",
      state.calibrationInProgress
    );
